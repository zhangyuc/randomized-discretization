...
def run_attack(checkpoint, x_adv, epsilon):
  ...
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      predictions = np.zeros([x_batch.shape[0], 10])
      for _ in range(testnum):
        predictions += sess.run(model.y_pred_one_hot, feed_dict=dict_adv)

      final_prediction = np.argmax(predictions, axis=1)
      cur_corr = np.sum(np.equal(final_prediction, y_batch).astype(np.float32))
